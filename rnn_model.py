...
#建立model
def create_rnn_model(layer,unit_list):
    timesteps=2
    if layer == 1:
        model = Sequential()
        model.add(SimpleRNN(units=unit_list[0], input_shape=(timesteps, 8)))
        model.add(Dense(units=7))  
    elif layer == 2:
        model = Sequential()
        model.add(SimpleRNN(units=unit_list[0], input_shape=(timesteps, 8),return_sequences=True))
        model.add(SimpleRNN(units=unit_list[1], input_shape=(timesteps, 8)))
        model.add(Dense(units=7))
    elif layer == 3:
        model = Sequential()
        model.add(SimpleRNN(units=unit_list[0], input_shape=(timesteps, 8),return_sequences=True))
        model.add(SimpleRNN(units=unit_list[1], input_shape=(timesteps, 8),return_sequences=True))
        model.add(SimpleRNN(units=unit_list[2], input_shape=(timesteps, 8)))
        model.add(Dense(units=7)) 
    return model

#訓練模型，並將結果寫入txt檔
def year1train(X,y,tt1,test_1i,features,target,layer,unit_list):
    f = open("rnn_result.txt",mode="a")
    all_time = []
    all_error = []
    for i in range(len(tt1)):
        ...
        model = create_rnn_model(layer,unit_list)
        ...
        print(exe_time)
        last_7_days = X_test[:2].reshape(1, 2, -1)
        predicted = model.predict(last_7_days)
        print(predicted)
        pre = list(predicted[0])
